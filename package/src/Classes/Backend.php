...
class Backend
{
    ...
    {
        // parse controller yaml files and store config data
        $controllers = [];
        ...
        $this->config = KeyedArray::from($controllers, 'id');

        // ensure no duplicate controller ids are present
        $ids = [];

        foreach ($this->config as $config) {
            if (in_array($config['id'], $ids)) {
                throw new DuplicateControllerIdException($config['id']);
            }

            $ids[] = $config['id'];
        }

        // instantiate controllers and store them in a collection
        $this->controllers = collect($this->config)->map(fn ($data) => Controller::create($data));
    }
     ...
     * @param string $id
     *
     * @return Bedard\Backend\Configuration\Controller|null
     */
    public function controller(string $id): Controller|null
    {
        return $this->controllers->first(fn ($ctrl) => $ctrl->get('id') === $id) ?? null;
    }
